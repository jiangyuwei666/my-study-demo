...
class ZhihuSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # 生成返回 关注我的人 界面的解析请求
        yield scrapy.Request(url=self.followers_url.format(user_token=self.start_user, include=self.followers_query, offset=0, limit=20), callback=self.parse_follower)
        # 测试用
    ...
    def parse_user(self, response):
        ...
        # 返回所有人的粉丝列表的请求到队列
        yield scrapy.Request(url=self.followers_url.format(user_token=user_result.get('url_token'), include=self.followers_query, offset=0, limit=20), callback=self.parse_follower)
    ...
    def parse_follow(self, response):
        ...
        if 'paging' in follow_result.keys():
            if follow_result.get('paging').get('is_end') is False:
                yield scrapy.Request(url=follow_result.get('paging').get('next').replace('https://www.zhihu.com/', 'https://www.zhihu.com/api/v4/'), callback=self.parse_follow)

    # 关注者界面（跟关注的人界面相似）
    def parse_follower(self, response):
        follower_result = json.loads(response.text)
        if 'data' in follower_result.keys():
            for i in follower_result.get('data'):
                yield scrapy.Request(url=self.user_info_url.format(user_token=i.get('url_token'), include=self.user_info_query), callback=self.parse_user)
